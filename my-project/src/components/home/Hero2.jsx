...
const InfoBlock = ({ title, description, image }) => (
  <div className="text-center">
    <img className="w-60 h-60 object-cover mx-auto" src={image} alt={title} />
    <h1 className="font-playfair mt-4 text-2xl">{title}</h1>
    <p className="font-playfair text-lg">{description}</p>
  </div>
);

const MenuCard = ({ name, price, originalPrice, rating, setRating, onClick }) => (
  <div
    className="card max-w-md bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden cursor-pointer"
    onClick={onClick}
  >
    <img className="w-full h-48 object-cover" src={menuImage} alt={name} />
    <div className="p-6">
      <h3 className="text-2xl font-semibold text-gray-800 border-b border-gray-200 pb-2 mb-4 text-center">
        {name}
      </h3>
      ...
      <div className="flex justify-center space-x-2 items-center">
        <span className="text-xl font-bold text-gray-800">{price} Fcfa</span>
...
export const Hero2 = () => {
  ...
  const openModal = (cardIndex) => {
    ...
    setIsOrdered(false); // Réinitialise l'état lorsque le modal s'ouvre
  };

  const closeModal = () => {
    setSelectedCard(null);
    setIsOrdered(false); // Réinitialise l'état lorsque le modal se ferme
  };

  const handleOrderClick = () => {
    setIsOrdered(true); // Change l'état pour afficher le message de confirmation
  };
  ...
  return (
    <div className="my-10 relative">
      ...
      <section className="mb-8">
        <h1 className="text-2xl font-noto-serif text-center">Pourquoi nous ?</h1>
        ...
        <div className="grid grid-cols-3 gap-8 mt-8 mx-auto max-w-4xl">
      ...
      <AnimatePresence>
        {selectedCard !== null && (
          ...
          >
            <div className="bg-white p-8 rounded-lg shadow-lg max-w-md mx-auto font-playfair">
              <button
                className="block ml-auto px-4 py-2 bg-slate-50 text-black border font-semibold rounded hover:bg-slate-300 transition duration-300 mb-4"
                onClick={closeModal}
              >
                <FaTimes/>
              </button>
              ...
              {isOrdered ? (
                ...
                <>
                  <img className="w-full h-48 object-cover mb-4" src={menuImage} alt="Nom du plat" />
                  <h2 className="text-2xl font-bold text-center mb-4 border-b mx-4 ">Nom du plat</h2>
                  ...
                  <div className="flex justify-center space-x-2 items-center mb-4">
                    <span className="text-xl font-bold text-gray-800">199990 Fcfa</span>
